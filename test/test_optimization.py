...
def test_roundtrip(sample_location, sample_time, particle, solar, dipole, kind):
    ...
    expected = aep8.flux(
        sample_location, sample_time, energy,
        kind=kind, solar=solar, particle=particle,
    )

    # Split: compute L/BBo, then look up flux
    L, BBo = aep8.magnetic_coordinates(
        sample_location, sample_time, dipole=dipole,
    )
    actual = aep8.flux_from_magnetic_coordinates(
        L, BBo, energy, kind=kind, solar=solar, particle=particle,
    )

    np.testing.assert_allclose(
        actual.value, expected.value, rtol=1e-10,
...
def test_broadcasting(sample_location, sample_time):
    ...
    L, BBo = aep8.magnetic_coordinates(
        sample_location, sample_time, dipole="JensenANDCain1960",
    )
...
def test_reuse_across_energies(sample_location, sample_time):
    ...
    for energy_val in [0.1, 1.0, 10.0]:
        ...
        result = aep8.flux_from_magnetic_coordinates(
            L, BBo, energy, kind="integral", solar="min", particle="e",
        )
...
def test_reuse_across_models(sample_location, sample_time):
    ...
    for particle, solar in [("e", "min"), ("e", "max"), ("p", "min")]:
        expected = aep8.flux(
            sample_location, sample_time, energy,
            kind="integral", solar=solar, particle=particle,
        )
        actual = aep8.flux_from_magnetic_coordinates(
            L, BBo, energy, kind="integral", solar=solar, particle=particle,
        )
        np.testing.assert_allclose(
            actual.value, expected.value, rtol=1e-10,
...
def test_scalar_location():
    ...
    result = aep8.flux_from_magnetic_coordinates(
        L, BBo, energy, kind="integral", solar="max", particle="p",
    )
    expected = aep8.flux(
        loc, time, energy, kind="integral", solar="max", particle="p",
    )
...
def test_vectorized_energy_roundtrip(simple_location, simple_time):
    ...
    result = aep8.flux(
        simple_location, simple_time, energy_array,
        kind="integral", solar="min", particle="e",
    )
    ...
    for i, e in enumerate(energies_MeV):
        scalar_result = aep8.flux(
            simple_location, simple_time, e * u.MeV,
            kind="integral", solar="min", particle="e",
        )
        np.testing.assert_allclose(
            result[..., i].value, scalar_result.value, rtol=1e-10,
...
def test_vectorized_energy_flux_from_mag_coords(simple_location, simple_time):
    ...
    L, BBo = aep8.magnetic_coordinates(
        simple_location, simple_time, dipole="JensenANDCain1960",
    )
...
def test_scalar_energy_backward_compat(simple_location, simple_time):
    ...
    result = aep8.flux(
        simple_location, simple_time, energy,
        kind="integral", solar="min", particle="e",
    )
...
def test_energy_chunking(simple_location, simple_time):
    ...
    result = aep8.flux(
        simple_location, simple_time, energies,
        kind="integral", solar="min", particle="e",
    )
    ...
    for idx in [0, 14, 29]:
        scalar_result = aep8.flux(
            simple_location, simple_time, energies[idx],
            kind="integral", solar="min", particle="e",
        )
        np.testing.assert_allclose(
            result[..., idx].value, scalar_result.value, rtol=1e-10,
